...
Entao("devo ver a seguinte mensagem:") do |doc_string|
  expect_message = doc_string.gsub("DATA_ATUAL", Time.now.strftime("%d/%m/%Y")) #Recurso do ruby para substituir texto
...
Entao("devo ver os links: {string} e {string} no pedido") do |button_accept, button_reject|
  expect(@dash_page.order_actions(button_accept && button_reject)).to be true
end
